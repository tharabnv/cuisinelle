...
function blossom_recipe_customizer_theme_info( $wp_customize ) {
    ...
    $theme_info = '<p>';
	$theme_info .= sprintf( __( 'Demo Link: %1$sClick here.%2$s', 'blossom-recipe' ),  '<a href="' . esc_url( 'https://blossomthemes.com/theme-demo/?theme=' . BLOSSOM_RECIPE_THEME_TEXTDOMAIN ) . '" target="_blank">', '</a>' );
    $theme_info .= '</p><p>';
	...
	$manager->add_section(
		new Blossom_Recipe_Customize_Section_Pro(
			...
			array(
				...
				'pro_url'  => esc_url( 'https://blossomthemes.com/wordpress-themes/blossom-recipe-pro/?utm_source=free_theme&utm_medium=customizer&utm_campaign=pro_theme_upgrade' ),
			)
		)
	);
}
